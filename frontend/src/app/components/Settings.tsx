...
import { Bell, Settings as SettingsIcon, Moon, Sun, Camera } from "lucide-react";
        ...
        <div className="space-y-6">
          <div>
            ...
            <input
              ...
              className={`w-full h-2 rounded-lg appearance-none cursor-pointer accent-blue-600 ${darkMode ? 'bg-gray-600' : 'bg-gray-200'
                }`}
            />
          ...
          <div className={`grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
            <div>
              ...
              <select className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${darkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300'
                }`}>
        ...
        <div className="space-y-6">
          <div className={`flex items-center justify-between p-4 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-50'
            ...
            <button
              ...
              className={`w-12 h-6 rounded-full transition-colors ${emailAlerts ? "bg-blue-600" : (darkMode ? "bg-gray-600" : "bg-gray-300")
                }`}
            >
              <div className={`w-5 h-5 bg-white rounded-full shadow-md transform transition-transform ${emailAlerts ? "translate-x-6" : "translate-x-0.5"
                }`} />
            </button>
          </div>
          ...
          <div>
            ...
            <select
              className={`w-full md:w-64 px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${darkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300'
                }`}
          ...
          <div className={`pt-4 border-t ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
            ...
            <div className="space-y-2">
              {['Weapon Detection', 'Blacklist Match', 'Unrecognized Person', 'Running Detected', 'Loitering Detected'].map((alertType) => (
                <label key={alertType} className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors ${darkMode ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-50 hover:bg-gray-100'
                  }`}>
                  <input
                    type="checkbox"
                    defaultChecked
      ...
      {cameras.length > 0 && (
        <div className={`rounded-xl shadow-sm p-6 border mb-6 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'
          ...
          <div className="space-y-4">
            {cameras.map(camera => (
              <div key={camera.id} className={`p-4 rounded-lg border ${darkMode ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'
                }`}>
