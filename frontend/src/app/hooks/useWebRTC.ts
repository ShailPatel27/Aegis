...
const WS_BASE = import.meta.env.VITE_BACKEND_WS_URL || BACKEND_BASE_URL.replace("https://", "wss://").replace("http://", "ws://");
...
export function useCameraStream(cameraId: string, localStream: MediaStream | null) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const wsUrl = `${WS_BASE}/api/webrtc/ws/camera/${cameraId}`;
...
export function useMonitorStream(cameraId: string) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const wsUrl = `${WS_BASE}/api/webrtc/ws/monitor/${cameraId}`;
